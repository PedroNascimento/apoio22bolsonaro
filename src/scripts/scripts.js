...
const pantonFont = new FontFace(
  'myPantonFont',
  'url(./src/assets/panton-extrabold.otf)'
)
...
baseImage.src = './public/img/apoio22bolsonaro_v3.png'
...
async function shareImage() {
  try {
    ...
    Alert('Não foi possível compartilhar a imagem: ' + (error.message || error))
  }
}
...
document.querySelector('input[name=yourname]').addEventListener('keyup', ev => {
  ev.preventDefault()
  ...
  context.translate(1080 / 2.5, 515)
  context.rotate((-6.9 * Math.PI) / 180)

  context.textBaseline = 'middle'
  context.font = '120px myPantonFont'
  ...
  context.fillText(document.querySelector('input').value.trim(), -width / 2, 0)
